...
class DailyExpenditureSummarySheetController extends Controller
{
    public function dailyExpenditure(){
        ...
        return view('report.daily_expenditure_summary',compact('projects'));
    }

    public function printDailyExpenditure(Request $request){
        ...
        //     WHERE (v.voucher_date  BETWEEN  '".$from_date."' AND '".$to_date."') AND l.ltype_id = 3 AND v.project_id = ".$project_id);

        $data['expen'] = DB::select("SELECT l.name as l_name,bk.name as bank_name,v.voucher_number as v_no, vd.amount FROM `lnames` AS l
        JOIN `ltypes` AS lt ON lt.id = l.ltype_id
        JOIN `voucher_details` AS vd ON vd.lname_id = l.id
        JOIN `vouchers` AS v ON v.id = vd.voucher_id
        JOIN `banks` AS bk ON bk.id = v.bank_id
        ...
        return view('print_report.print_expenditure_summary_sheet',$data);
    }
}
